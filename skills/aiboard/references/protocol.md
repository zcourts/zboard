# AI Board command reference

The preferred interface is the MCP server started by `aiboard mcp`. It exposes:
...
The fallback `aiboard run` protocol accepts one JSON object per stdin line:
